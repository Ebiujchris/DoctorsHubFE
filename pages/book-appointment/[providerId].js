...
export default function BookAppointment() {
  ...
  // Fetch provider data when providerId is available
  useEffect(() => {
    ...
    const fetchProvider = async () => {
      try {
        ...
        const response = await fetch(`${API_BASE}/users/${providerId}`)
        ...
        setProvider({
          ...
          specialty: data.specialty || data.role,
          rating: data.rating || 4.8,
          reviews: data.reviews || 0,
          experience: data.experience || 'N/A',
          responseTime: data.responseTime || '< 2 hours',
          image: data.profilePicture || `https://i.pravatar.cc/150?img=${Math.random() * 50}`
        })
      } catch (error) {
        console.error(error)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('📤 API endpoint:', `${API_BASE}/bookings`)
      console.log('📤 User ID from token:', token ? JSON.parse(atob(token.split('.')[1])).sub : 'UNABLE_TO_PARSE')
      ...
      if (!response.ok) {
        ...
        throw new Error(responseData.message || `Booking failed: ${response.status}`)
      }
      ...
      <div className="min-h-screen bg-slate-50">
        ...
        <main className="max-w-4xl mx-auto p-6">
          <div className="bg-red-50 border border-red-200 rounded-xl p-8 text-center">
            <p className="text-red-800 text-lg font-semibold">Provider not found</p>
            <button
              ...
              <div className="bg-white rounded-xl shadow-lg p-8">
                ...
                {!getAuthToken() && (
                  <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
                    <p className="text-red-800 font-semibold text-sm mb-2">⚠️ Not Authenticated</p>
                    <p className="text-red-700 text-sm mb-3">Your session is not valid. Please login again to continue with the booking.</p>
                    <button
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="flex gap-4 pt-4">
                    ...
                    <button
                      type="submit"
                      disabled={bookingLoading || !getAuthToken()}
                      className="flex-1 px-6 py-3 bg-gradient-to-r from-indigo-600 to-blue-600 text-white font-semibold rounded-lg hover:from-indigo-700 hover:to-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-md hover:shadow-lg"
                    >
                      {!getAuthToken() ? 'Please Login to Book' : bookingLoading ? 'Submitting...' : 'Request Appointment'}
                    </button>
                  </div>
